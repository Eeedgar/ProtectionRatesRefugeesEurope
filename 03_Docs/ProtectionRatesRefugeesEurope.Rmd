---
title: "Protection rates of refugees in Europe"
author: "Edgar Scrase"
date: "22 August 2018"
output: 
  html_document:
...
Here is the output and the next sections go through how it was produced step-by-step.  All the [source code and data](https://github.com/Eeedgar/ProtectionRatesRefugeesEurope) are available on GitHub.

![](../02_OutputVisualisation/Vis.png)
...
There are three different statuses for refugees in Europe, ranging from Geneva convention through to humanitarian and subsidiary statuses, and refugees' rights vary considerably with each type of status.  Here is more information about [what is a refugee](http://www.unhcr.org/what-is-a-refugee.html) from the United Nations Refugee Agency. 
...
1. **Statistical data** - is from [Eurostat](http://ec.europa.eu/eurostat/web/asylum-and-managed-migration/data/database?p_p_id=NavTreeportletprod_WAR_NavTreeportletprod_INSTANCE_sFp6GUtIbBHg&p_p_lifecycle=0&p_p_state=normal&p_p_mode=view&p_p_col_id=column-2&p_p_col_count=1) and is available via an API from EuroStat.  If you can't wait to download the data, use this already downloaded [raw data](https://github.com/Eeedgar/ProtectionRatesRefugeesEurope/tree/master/01_RawData).  The asylum decision data is available on a quarterly basis.  
2. **Country spatial data** - in this case the *centroid* (which is the geographical centre) was scraped from a [Google documentation page](https://developers.google.com/public-data/docs/canonical/countries_csv).
3. **Basemap** - the [tiles](https://api.mapbox.com/styles/v1/edgarscrase/cjl1c78tn37v72sofn80jo9en.html?fresh=true&title=true&access_token=pk.eyJ1IjoiZWRnYXJzY3Jhc2UiLCJhIjoiY2pram90c3M3MWRxdjNxcWhzOXRzY3N6ZCJ9._HQxSBcViAYVr9Bg1OWI_A#3.4/57.750060/17.037912/0) were produced using OpenStreetMap data and were styled in MapBox Studio and then published.  MapBox Studio gives you a lot of flexibility to easily creating your ideal base map.
...
![](../03_Docs/WhatNotToDo_Piecharts_on_map.jpg)
...
```{r libraries, warning=F, message=F, echo=T}
...
Lets use tribble to get the table of country spatial data from this [Google documentation page](https://developers.google.com/public-data/docs/canonical/countries_csv).

```{r loading, warning=F, message=F, echo=T}
...
```
```{r loading0Actual, warning=F, message=F, echo=F}
...
```{r loading1, warning=F, message=F, echo=T}
...
``` {r loading3, warning=F, message=F, echo=T}
...
``` {r loading4, warning=F, message=F, echo=T}
...
``` {r loading5, warning=F, message=F, echo=T}
if(doRun==TRUE) {
...
``` {r loading6, warning=F, message=F, echo=T}
...
``` {r transform1, warning=F, message=F, echo=T}
#-------------------------------------------------------------------------------------------------------
# Worker functions ...
#-------------------------------------------------------------------------------------------------------
...
``` {r transform2, warning=F, message=F, echo=T}
#-------------------------------------------------------------------------------------------------------
...
``` {r visualise1, warning=F, message=F, echo=T}
#-------------------------------------------------------------------------------------------------------
...
``` {r visualise2, warning=F, message=F, echo=T}
#-------------------------------------------------------------------------------------------------------
...
``` {r visualise3, warning=F, message=F, echo=T}
#-------------------------------------------------------------------------------------------------------
# Summary 3 - the % of each decision type by country and broken out by country of origin
...
``` {r visualise4, warning=F, message=F, echo=T}
#-------------------------------------------------------------------------------------------------------
...
``` {r visualise5, warning=F, message=F, echo=T}
...
``` {r visualise6, warning=F, message=F, echo=T}
